...
public class PriceAndMACrossingOptimizer {
	
	final static class SimpleGAParams {
		final int maLength;
		final int maLengthClose;
		
		final static int MA_MAX_LENGTH = 100;
		final static int MA_MIN_LENGTH = 2;
		
		SimpleGAParams(final int maLength, final int maLengthClose)
		{
			this.maLength = maLength;
			this.maLengthClose = maLengthClose;
		}
	}
	
	private static final Logger log = LoggerFactory.getLogger(PriceAndMACrossingOptimizer.class);
	...
	private static double maxProfit(final SimpleGAParams params)
	{
		...
		return smacs.goBackTestForProfits(elemTable.getOHCLElementsSortedByDate(), Arrays.asList(params.maLength, params.maLengthClose), null, Arrays.asList(isLong));
//		return smacs.goBackTestForPercentageProfit(elemTable.getOHCLElementsSortedByDate(), Arrays.asList(params.maLength, params.maLengthClose), null, Arrays.asList(isLong));
	}
	...
	public static void main(String[] args) {
		...
		final Engine<IntegerGene, Double> engine = Engine
				.builder(
					PriceAndMACrossingOptimizer::maxProfit,
					codec)
